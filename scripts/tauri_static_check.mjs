...
const checks = [
  ...
  ["settings page contains no obsolete note configuration", views.includes("settings-window") && !views.includes("stayLightweight") && !state.includes("stayLightweight")],
  ["autostart uses the concise boot label", state.includes('startAtLogin: "开机自动启动"') && !views.includes("startAtLoginHint")],
  ...
  ["direct updates remain fixed-source, bounded, and parent-exit safe", rust.includes("direct_update::download_and_install_update") && directUpdate.includes("RELEASE_DOWNLOAD_BASE") && directUpdate.includes("--connect-timeout") && directUpdate.includes("--max-time") && directUpdate.includes("--ssl-no-revoke") && directUpdate.includes("WaitForExit") && directUpdate.includes("-ArgumentList @('/S')")],
  ...
  ["release publishes installers and legacy update metadata", release.includes("bundle/nsis/*.exe") && release.includes("bundle/dmg/*.dmg") && release.includes("My.Sticky.Notes.Setup.${version}.exe") && release.includes("My Sticky Notes_${version}_x64-setup.exe") && release.includes("SHA256SUMS.txt")],
